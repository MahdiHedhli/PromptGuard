...
The detection numbers in [docs/benchmarks.md](docs/benchmarks.md) come from real public corpora: [AI4Privacy PII-Masking-300k](https://huggingface.co/datasets/ai4privacy/pii-masking-300k) (English validation split, 7,946 records, 22,624 in-scope spans) and the [GitHub Issues Secrets Benchmark](https://zenodo.org/records/17430336) (arxiv 2410.23657, test_wild split, 1,488 records, 29 real secrets). The synthetic corpus at `benchmarks/run_detection_benchmarks.py` is kept as a development-time regression target only; its numbers are not the credibility claim.

AI4Privacy is licensed for academic evaluation; the GitHub Issues Secrets dataset is CC-BY 4.0. We use both for evaluation only, do not redistribute either, and do not train on either. SHA256 manifests of the cached files live at `local/benchmarks/corpora/<name>/manifest.txt` so the methodology is reproducible. See [docs/benchmarks.md](docs/benchmarks.md) for the per-category three-way comparison (bare LiteLLM baseline / PromptGuard regex-only / PromptGuard full pipeline) and for the calibration boundary surfaced on vendor-specific secrets.
